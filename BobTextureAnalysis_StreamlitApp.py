...
with col2:
    if st.session_state.opencv_image is not None:
        ...
        coherence, two_phi = coh_ang_calc(raw_image)

        all_img = orient_hsv(raw_image, coherence, two_phi, mode='all')
        coh_img = orient_hsv(raw_image, coherence, two_phi, mode='coherence')
        ang_img = orient_hsv(raw_image, coherence, two_phi, mode='angle')
